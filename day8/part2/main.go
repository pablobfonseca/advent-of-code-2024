	...
	for _, points := range grid.positions {
		for i := 0; i < len(points); i++ {
			for j := i + 1; j < len(points); j++ {
				...
				for k := -len(grid.data); k <= len(grid.data); k++ {
					antinode1 := Point{x: p1.x + dx*k, y: p1.y + dy*k}
					antinode2 := Point{x: p2.x + dx*k, y: p2.y + dy*k}

					if grid.isInBounds(antinode1) && grid.isInBounds(antinode2) {
